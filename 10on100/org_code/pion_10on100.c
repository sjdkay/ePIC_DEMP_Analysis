void pion_10on100()
{
 ...
 //-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  // else if (i == 1) {
	   filepath = Form("root://dtn-eic.jlab.org//work/eic2/EPIC/RECO/24.12.0/epic_craterlake/EXCLUSIVE/DEMP/DEMPgen-1.2.2/10x100/q2_%i_%i/pi+/DEMPgen-1.2.2_10x100_pi+_q2_%i_%i.%04i.eicrecon.tree.edm4eic.root", i+9, i+19, i+9, i+19, j);
     // }
...
/*  TChain *mychain = new TChain("events");
  ...
  unsigned int nFiles1 = 1; // Number of iterations for the first loop //4
  unsigned int nFiles2 = 41; // Number of iterations for the second loop //484
  unsigned int nFiles; // Total number of files to process
  unsigned int nEntries;            // Variable to hold the number of entries in the tree

  for (unsigned int i = 0; i < nFiles1; i++) {
  //  if(i == 0) {nFiles2 = 80;}
   // else if(i == 1) {nFiles2 = 41;}
   // else if(i == 2) {nFiles2 = 91;}
    ...
    //TString fileName = Form("/volatile/eic/preet/reco_simulation_output/Dec2024_Files/pi+/10on100/Test_10on100_pi+_Reco_AllEvents_%i_%i.root", 1 + i, 5000 + j * 5000);
      TString fileName = Form("/volatile/eic/preet/reco_simulation_output/Dec2024_Files/pi+/new_files/10on100/Test_10on100_pi+_Reco_AllEvents_%i_%i.root", 2 + i, 5000 + j * 5000);
...
//	std::cout << "nEntries = " << nEntries << std::endl;
 ...
 // nFiles = 1.0;
  ...
  //-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // Define Histograms
  TH2* enbfTruthw_Thetap = new TH2D("enbfTruthw_Thethap","e' truth no beam effects #theta vs P; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,135,175,100,9.5,11);
  TH2* pinbfTruthw_Thetap = new TH2D("pinbfTruthw_Thethap","#pi^{+} truth no beam effects #theta vs P; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,0,50,100,0,80); //change
  TH2* nnbfTruthw_Thetap  = new TH2D("nnbfTruthw_Thethap","n truth no beam effects #theta vs P; #theta (Deg); P (GeV/c); Rate/bin (Hz)",100,0.0,3.5,100,20,100); //change 
  
  TH2* eTruthw_Thetap  = new TH2D("eTruthw_Thethap","e' truth #theta vs P; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,135,175,100,9.5,11);
  TH2* piTruthw_Thetap = new TH2D("piTruthw_Thethap","#pi^{+} truth #theta vs P; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,0,50,100,0,80); //change
  TH2* nTruthw_Thetap  = new TH2D("nTruthw_Thethap","n truth #theta vs P; #theta (Deg); P (GeV/c); Rate/bin (Hz)",100,0.0,3.5,100,20,100); //change 
  ...
  TH2* nTruthw_rot_Thetap  = new TH2D("nTruthw_rot_Thethap","n truth #theta* vs P around p axis; #theta* (Deg); P (GeV/c); Rate/bin (Hz)",100,0.0,1.0,100,20,100); //change
  TH2* nTruthw_rot_Thetaphi  = new TH2D("nTruthw_rot_Thethaphi","n truth #theta* vs #phi* around p axis; #theta* (mRad); #phi* (deg); Rate/bin (Hz)",100,0.0,15,100,-200,200); //change

  TH2* eRecw_Thetap  = new TH2D("eRecw_Thethap","e' rec #theta vs P; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,135,175,100,7,12);
  TH2* eRecw_Thetaphi  = new TH2D("eRecw_Thethaphi","e' rec #theta vs #phi; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,135,175,100,-200,200);
  TH2* piRecw_Thetap = new TH2D("piRecw_Thethap","#pi^{+} rec #theta vs P; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,0,50,100,0,120); //change
  ...
  TH1* nRec_en = new TH1D("nRec_en", "n rec E for all clusters ( rec #theta* < 4.0 mRad ); E (GeV); Rate (Hz)", 100, 0.0, 120); //change
  ...
  TH1* eRecw_Eta_Cut = new TH1D("eRecw_Eta_Cut", "e' #eta for detected events; #eta; Rate (Hz)",100,-3,-1);
  eRecw_Eta_Cut->SetLineWidth(2);
  TH1* eEff_Eta = new TH1D("eEff_Eta", "e' Tracking efficiency as fn of #eta; #eta; Eff", 100,-3,-1);
  ...
  TH1* eTruthw_P_Uncut = new TH1D("eTruthw_P_Uncut", "e' P for thrown events; P (GeV/c); Rate (Hz)",100,9.5,11);
  eTruthw_P_Uncut->SetLineWidth(2);
  TH1* eRecw_P_Cut = new TH1D("eRecw_P_Cut", "e' P for detected events; P (GeV/c); Rate (Hz)",100,9.5,11);
  eRecw_P_Cut->SetLineWidth(2);
  TH1* eEff_P = new TH1D("eEff_P", "e' Tracking efficiency as fn of P; P (GeV/c); Eff", 100,9.5,11);
  eEff_P->SetLineWidth(2);

  TH1* piTruthw_P_Uncut = new TH1D("piTruthw_P_Uncut", "#pi^{+} P for thrown events; P (GeV/c); Rate (Hz)",100,-1,80); //change
  piTruthw_P_Uncut->SetLineWidth(2);
  TH1* piRecw_P_Cut = new TH1D("piRecw_P_Cut", "#pi^{+} P for detected events; P (GeV/c); Rate (Hz)",100,-1,80); //change
  piRecw_P_Cut->SetLineWidth(2);
  TH1* piEff_P = new TH1D("piEff_P", "#pi^{+} Tracking efficiency as fn of P; P (GeV/c); Eff",100,-1,80); //change
  piEff_P->SetLineWidth(2);

  // Neutrons in HCal
  TH2* nRecw_Thetap_hcal  = new TH2D("nRecw_Thethap_hcal","n rec #theta vs P for 1 cluster events; #theta (Deg); P (GeV/c); Rate/bin (Hz)",100,0.8,2.0,100,0.0,100); //change
  TH2* nRecw_rot_Thetap_hcal  = new TH2D("nRecw_rot_Thethap_hcal","n rec #theta* vs P around p axis for 1 cluster events ( rec #theta* < 4.0 mRad, E > 40 GeV ); #theta* (mRad); P (GeV/c); Rate/bin (Hz)",100,0,4.5,100,35.0,100); //change
    
  // y plot 
  TH1* htw_Truth_y = new TH1D("htw_Truth_y", "y truth Distribution; y; Rate (Hz) ", 100,-0.3,0.3);
  ...
  TH1* htw_Truth_t = new TH1D("htw_Truth_t", "-t truth Distribution; -t (GeV^{2}); Rate (Hz) ", 100,-0.1,1.5);
  ...
  while(tree_reader.Next()) { // Loop over events
    ...
    t_nbfalttruth = -1*(tnbfalttruth.mag2()); // t_alttruth is the -t from the second loop for nbf
     ...
     htw_Truth_t -> Fill(t_truth, weight); //htw_Truth_t -> Fill(t_nbfalttruth, weight);
    // Efficiency plots
  ...
  c1a->Print("DEMP_reco_10on100.pdf");
  c2a->Print("DEMP_reco_10on100.pdf");
  c3_a->Print("DEMP_reco_10on100.pdf");
  c1->Print("DEMP_reco_10on100.pdf");
  c2->Print("DEMP_reco_10on100.pdf");
  c3b->Print("DEMP_reco_10on100.pdf");
  c3->Print("DEMP_reco_10on100.pdf");
  c3a->Print("DEMP_reco_10on100.pdf");
  c3c->Print("DEMP_reco_10on100.pdf");
  c4->Print("DEMP_reco_10on100.pdf");
  c4a->Print("DEMP_reco_10on100.pdf");
  c5->Print("DEMP_reco_10on100.pdf");
  c5_a->Print("DEMP_reco_10on100.pdf");
  c5a->Print("DEMP_reco_10on100.pdf");
  c5b->Print("DEMP_reco_10on100.pdf");
  c5c->Print("DEMP_reco_10on100.pdf");
  c5d->Print("DEMP_reco_10on100.pdf");
  c6b->Print("DEMP_reco_10on100.pdf");
  c6a->Print("DEMP_reco_10on100.pdf");
  c7->Print("DEMP_reco_10on100.pdf");
  c7a->Print("DEMP_reco_10on100.pdf");
  c7b->Print("DEMP_reco_10on100.pdf");
  c8->Print("DEMP_reco_10on100.pdf");
  c8a->Print("DEMP_reco_10on100.pdf");
  c9->Print("DEMP_reco_10on100.pdf");
  c9a->Print("DEMP_reco_10on100.pdf");
  c10->Print("DEMP_reco_10on100.pdf");
  c10a->Print("DEMP_reco_10on100.pdf");
  c11->Print("DEMP_reco_10on100.pdf");
  c11a->Print("DEMP_reco_10on100.pdf");
  c12->Print("DEMP_reco_10on100.pdf");
  c13->Print("DEMP_reco_10on100.pdf");
  c14a->Print("DEMP_reco_10on100.pdf");
  c14b->Print("DEMP_reco_10on100.pdf");
  c14c->Print("DEMP_reco_10on100.pdf");
  c14d->Print("DEMP_reco_10on100.pdf");
  c15a->Print("DEMP_reco_10on100.pdf");
  c15b->Print("DEMP_reco_10on100.pdf");
  c15c->Print("DEMP_reco_10on100.pdf");
  c15d->Print("DEMP_reco_10on100.pdf");
  c15e->Print("DEMP_reco_10on100.pdf");
  c15f->Print("DEMP_reco_10on100.pdf");
  c16f->Print("DEMP_reco_10on100.pdf");
  c16d->Print("DEMP_reco_10on100.pdf");
  c16e->Print("DEMP_reco_10on100.pdf");
  c16a->Print("DEMP_reco_10on100.pdf");
  c16b->Print("DEMP_reco_10on100.pdf");
  c16c->Print("DEMP_reco_10on100.pdf");
  c17->Print("DEMP_reco_10on100.pdf");
  c18b->Print("DEMP_reco_10on100.pdf");
  c18a->Print("DEMP_reco_10on100.pdf");
  c18->Print("DEMP_reco_10on100.pdf");
  c19a->Print("DEMP_reco_10on100.pdf");
  c19b->Print("DEMP_reco_10on100.pdf");
  c19->Print("DEMP_reco_10on100.pdf");
  c20a->Print("DEMP_reco_10on100.pdf");
  c20b->Print("DEMP_reco_10on100.pdf");
  c20->Print("DEMP_reco_10on100.pdf");
  c21a->Print("DEMP_reco_10on100.pdf");
  c21b->Print("DEMP_reco_10on100.pdf");
  c21->Print("DEMP_reco_10on100.pdf");
  c22a->Print("DEMP_reco_10on100.pdf");
  c22b->Print("DEMP_reco_10on100.pdf");
  c22->Print("DEMP_reco_10on100.pdf");
  c23a->Print("DEMP_reco_10on100.pdf");
  c23b->Print("DEMP_reco_10on100.pdf");
  c23c->Print("DEMP_reco_10on100.pdf");
  for(int D = 0; D <8; D++) { 
    c24[D]->Print("DEMP_reco_10on100.pdf"); }
  for(int D = 0; D <8; D++) {
    c25[D]->Print("DEMP_reco_10on100.pdf");};
  for(int D = 0; D <8; D++) {
    c26[D]->Print("DEMP_reco_10on100.pdf");};
  c27a->Print("DEMP_reco_10on100.pdf");
  c27b->Print("DEMP_reco_10on100.pdf");
  c28a->Print("DEMP_reco_10on100.pdf");
  c28b->Print("DEMP_reco_10on100.pdf");
  c28c->Print("DEMP_reco_10on100.pdf");
  c28d->Print("DEMP_reco_10on100.pdf");
  c28e->Print("DEMP_reco_10on100.pdf");
  c29->Print("DEMP_reco_10on100.pdf");
  c30a->Print("DEMP_reco_10on100.pdf");
  c1a->Print("DEMP_reco_10on100.pdf]");
  ...
  ofstream myfile;
  myfile.open("parameters_10on100.csv", std::ios::trunc);
  ...
  std::cout << "nEntries = " << mychain->GetEntries() << std::endl;

  double err_full;
  double intg_full = htw_Truth_t->IntegralAndError(0, (htw_Truth_t->FindFixBin(1.5)), err_full, "");
  cout<<"integral_full = "<<intg_full<<", error_full = "<<err_full<<endl;
  //-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
