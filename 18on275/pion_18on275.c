void pion_18on275()
{
  ...
  /*TChain *mychain = new TChain("events");
    ...
    if (i == 0) { filepath = Form("root://dtn-eic.jlab.org//work/eic2/EPIC/RECO/24.12.0/epic_craterlake/EXCLUSIVE/DEMP/DEMPgen-1.2.2/18x275/q2_%i_%i/pi+/DEMPgen-1.2.2_18x275_pi+_q2_%i_%i.%04i.eicrecon.tree.edm4eic.root", i+3, i+10, i+3, i+10, j);}
    else if (i == 1) { filepath = Form("root://dtn-eic.jlab.org//work/eic2/EPIC/RECO/24.12.0/epic_craterlake/EXCLUSIVE/DEMP/DEMPgen-1.2.2/18x275/q2_%i_%i/pi+/DEMPgen-1.2.2_18x275_pi+_q2_%i_%i.%04i.eicrecon.tree.edm4eic.root", i+9, i+19, i+9, i+19, j);}
    else { filepath = Form("root://dtn-eic.jlab.org//work/eic2/EPIC/RECO/24.12.0/epic_craterlake/EXCLUSIVE/DEMP/DEMPgen-1.2.2/18x275/q2_%i_%i/pi+/DEMPgen-1.2.2_18x275_pi+_q2_%i_%i.%04i.eicrecon.tree.edm4eic.root", i+18, i+33, i+18, i+33, j);} 
  ...
  */
  ...
  unsigned int nFiles1 = 9; // Number of iterations for the first loop //9
  unsigned int nFiles2; // Number of iterations for the second loop //164
  unsigned int nFiles = 9.0; // Total number of DEMPgen files added
  unsigned int nEntries; // Variable to hold the number of entries in the tree
  ...
  for (unsigned int i = 0; i < nFiles1; i++) {
    ...
    for (unsigned int j = 0; j < nFiles2; j++) {
      ...
      // Construct file name
      //TString fileName = Form("Test_18on275_pi+_Reco_AllEvents_%i_%i.root", 1 + i, 5000 + j * 5000);
     
      //Files from Jlab server
      //TString fileName = Form("/volatile/eic/preet/reco_simulation_output/Nov2024_Files/pi+/18on275/Test_18on275_pi+_Reco_AllEvents_%i_%i.root", 1 + i, 5000 + j * 5000);
      TString fileName = Form("/volatile/eic/preet/reco_simulation_output/Dec2024_Files/pi+/18on275/Test_18on275_pi+_Reco_AllEvents_%i_%i.root", 1 + i, 5000 + j * 5000);
  ...
  TFile *ofile = TFile::Open("pion_hist_18on275.root","RECREATE");
  ...
  //-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
  // Define Histograms
  TH2* enbfTruthw_Thetap = new TH2D("enbfTruthw_Thethap","e' truth no beam effects #theta vs P; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,150,180,100,17,19);
  TH2* pinbfTruthw_Thetap = new TH2D("pinbfTruthw_Thethap","#pi^{+} truth no beam effects #theta vs P; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,0,30,100,0,200); //change
  TH2* nnbfTruthw_Thetap  = new TH2D("nnbfTruthw_Thethap","n truth no beam effects #theta vs P; #theta (Deg); P (GeV/c); Rate/bin (Hz)",100,0.0,1.5,100,80,280);//change 
  
  TH2* eTruthw_Thetap  = new TH2D("eTruthw_Thethap","e' truth #theta vs P; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,150,180,100,17,19);
  TH2* piTruthw_Thetap = new TH2D("piTruthw_Thethap","#pi^{+} truth #theta vs P; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,0,30,100,0,200); //change
  TH2* nTruthw_Thetap  = new TH2D("nTruthw_Thethap","n truth #theta vs P; #theta (Deg); P (GeV/c); Rate/bin (Hz)",100,0.0,3.5,100,80,280); //change 
  TH2* nTruthw_Thetaphi  = new TH2D("nTruthw_Thethaphi","n truth #theta vs #phi; #theta (mRad); #phi (deg); Rate/bin (Hz)",100,0.0,50,100,-200,200); //change
  TH2* nTruthw_rot_Thetap  = new TH2D("nTruthw_rot_Thethap","n truth #theta* vs P around p axis; #theta* (Deg); P (GeV/c); Rate/bin (Hz)",100,0.0,1.0,100,80,280); //change
  TH2* nTruthw_rot_Thetaphi  = new TH2D("nTruthw_rot_Thethaphi","n truth #theta* vs #phi* around p axis; #theta* (mRad); #phi* (deg); Rate/bin (Hz)",100,0.0,10,100,-200,200); //change

  TH2* eRecw_Thetap  = new TH2D("eRecw_Thethap","e' rec #theta vs P; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,150,180,100,15,20);
  TH2* eRecw_Thetaphi  = new TH2D("eRecw_Thethaphi","e' rec #theta vs #phi; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,150,180,100,-200,200);
  TH2* piRecw_Thetap = new TH2D("piRecw_Thethap","#pi^{+} rec #theta vs P; #theta (deg); P (GeV/c); Rate/bin (Hz)",100,0,30,100,0,200); //change
  TH2* piRecw_Thetaphi  = new TH2D("piRecw_Thethaphi","#pi^{+} rec #theta vs #phi; #theta (deg); Rate/bin (Hz); P (GeV/c)",100,0,30,100,-200,200); //change
  ...
  TH2* nRecw_Thetap  = new TH2D("nRecw_Thethap","n rec #theta vs P for all clusters; #theta (Deg); P (GeV/c); Rate/bin (Hz)",100,0.8,2.0,100,0.0,300); //change
  TH2* nRecw_rot_Thetap  = new TH2D("nRecw_rot_Thethap","n rec #theta* vs P around p axis for all clusters ( rec #theta* < 4.0 mRad, E > 120 GeV ); #theta* (mRad); P (GeV/c); Rate/bin (Hz)",100,0.0,4.5,100,120,300); //change
  ...
  TH2* nRecw_rot_PosXY  = new TH2D("nRecw_rot_PosXY","n X vs Y around proton axis at Z = 35 m for all clusters ( rec #theta* < 4.0 mRad, E > 120 GeV ); x (mm); y (mm); Rate/bin (Hz)",100,-200,200,100,-200,200);


  TH1* nRec_en = new TH1D("nRec_en", "n rec E for all clusters ( rec #theta* < 4.0 mRad ); E (GeV); Rate (Hz)", 100, 0.0, 300); //change
  ...
  TH1* n_PhiDiff = new TH1D("n_PhiDiff", " #phi*_{pMiss_rec} - #phi*_{ZDC}; #phi*_{pMiss_rec} - #phi*_{ZDC}(Deg); Rate (Hz)", 100, -200, 200);
  n_PhiDiff->SetLineWidth(2);
  TH2* n_ThetaPhiDiff = new TH2D("n_ThetaPhiDiff", "#theta*_{pMiss_rec} - #theta*_{ZDC} vs #phi*_{pMiss_rec} - #phi*_{ZDC}; #theta*_{pMiss_rec} - #theta*_{ZDC} (Deg); #phi*_{pMiss_rec} - #phi*_{ZDC} (Deg); Rate/bin (Hz)",100, -1.0, 1.0, 100, -120, 120);
  ...
  TH1* htw_res_pi = new TH1D("htw_res_pi", "#pi^{+} Track Momentum Resolution Distribution (%); (P_{rec} - P_{MC})/P_{MC} (%); Rate (Hz)", 100, -30, 30);
  ...
  TH1* htw_res6 = new TH1D("htw_res6", "W Resolution Distribution (%); (W_{rec} - W_{truth})/W_{truth} (%); Rate (Hz)", 100, -100, 300); //change
  ...
  TH1* eTruthw_Eta_Uncut = new TH1D("eTruthw_Eta_Uncut", "e' #eta for thrown events; #eta; Rate (Hz)",100,-3.5,-1.5);
  eTruthw_Eta_Uncut->SetLineWidth(2);
  TH1* eRecw_Eta_Cut = new TH1D("eRecw_Eta_Cut", "e' #eta for detected events; #eta; Rate (Hz)",100,-3.5,-1.5);
  eRecw_Eta_Cut->SetLineWidth(2);
  TH1* eEff_Eta = new TH1D("eEff_Eta", "e' Tracking efficiency as fn of #eta; #eta; Eff", 100,-3.5,-1.5);
  eEff_Eta->SetLineWidth(2);

  TH1* piTruthw_Eta_Uncut = new TH1D("piTruthw_Eta_Uncut", "#pi^{+} #eta for thrown events; #eta; Rate (Hz)",100,1,7); //change
  piTruthw_Eta_Uncut->SetLineWidth(2);
  TH1* piRecw_Eta_Cut = new TH1D("piRecw_Eta_Cut", "#pi^{+} #eta for detected events; #eta; Rate (Hz)",100,1,7); //change
  piRecw_Eta_Cut->SetLineWidth(2);
  TH1* piEff_Eta = new TH1D("piEff_Eta", "#pi^{+} Tracking efficiency as fn of #eta; #eta; Eff",100,1,7); //change
  piEff_Eta->SetLineWidth(2);

  TH1* eTruthw_P_Uncut = new TH1D("eTruthw_P_Uncut", "e' P for thrown events; P (GeV/c); Rate (Hz)",100,17,19);
  eTruthw_P_Uncut->SetLineWidth(2);
  TH1* eRecw_P_Cut = new TH1D("eRecw_P_Cut", "e' P for detected events; P (GeV/c); Rate (Hz)",100,17,19);
  eRecw_P_Cut->SetLineWidth(2);
  TH1* eEff_P = new TH1D("eEff_P", "e' Tracking efficiency as fn of P; P (GeV/c); Eff", 100,17,19);
  eEff_P->SetLineWidth(2);

  TH1* piTruthw_P_Uncut = new TH1D("piTruthw_P_Uncut", "#pi^{+} P for thrown events; P (GeV/c); Rate (Hz)",100,-1,200); //change
  piTruthw_P_Uncut->SetLineWidth(2);
  TH1* piRecw_P_Cut = new TH1D("piRecw_P_Cut", "#pi^{+} P for detected events; P (GeV/c); Rate (Hz)",100,-1,200); //change
  piRecw_P_Cut->SetLineWidth(2);
  TH1* piEff_P = new TH1D("piEff_P", "#pi^{+} Tracking efficiency as fn of P; P (GeV/c); Eff",100,-1,200); //change
  piEff_P->SetLineWidth(2);

  // Neutrons in HCal
  TH2* nRecw_Thetap_hcal  = new TH2D("nRecw_Thethap_hcal","n rec #theta vs P for 1 cluster events; #theta (Deg); P (GeV/c); Rate/bin (Hz)",100,0.8,2.0,100,0.0,280); //change
  TH2* nRecw_rot_Thetap_hcal  = new TH2D("nRecw_rot_Thethap_hcal","n rec #theta* vs P around p axis for 1 cluster events ( rec #theta* < 4.0 mRad, E > 120 GeV ); #theta* (mRad); P (GeV/c); Rate/bin (Hz)",100,0,4.5,100,120,280); //change
    
  // y plot 
  TH1* htw_Truth_y = new TH1D("htw_Truth_y", "y truth Distribution; y; Rate (Hz) ", 100,-0.05,0.05);
  htw_Truth_y->SetLineWidth(2);
  TH1* htw_Rec_y = new TH1D("htw_Rec_y", "y rec Distribution; y; Rate (Hz) ", 100,-0.3,0.3);
  ...
  TH2* h2Recw_W_Q2  = new TH2D("h2Recw_W_Q2","W vs Q^{2} rec distribution; W (GeV); Q^{2} (GeV^{2})",100,-200,200,100,0,40);
  ...
  //w plot
  TH1* htw_Rec_w = new TH1D("htw_Rec_w", "w rec Distribution w/ 5 < Q^{2} < 35; W (GeV)", 100,-200,200);
  ...
  double ThetaDiff_Cut_Low = -0.07; // change based on the reconstructed plots of neutron theta vs phi 
  double ThetaDiff_Cut_High = 0.17;
  double PhiDiff_Cut = 80.0;
  ...
  while(tree_reader.Next()) { // Loop over events
	...
	nRec_en -> Fill(neut_rot_rec.E(), weight);
   	...
   	if(neut_rot_rec.E()>120){ // neutron energy cut //change
	  z = true;
	...
	if(neut_rot_rec_hcal.Theta()*1000. < 4.0 && neut_rot_rec_hcal.E()> 120){ //change
  ...
  c1a->Print("DEMP_reco_18on275.pdf[");
  c1a->Print("DEMP_reco_18on275.pdf");
  c2a->Print("DEMP_reco_18on275.pdf");
  c3_a->Print("DEMP_reco_18on275.pdf");
  c1->Print("DEMP_reco_18on275.pdf");
  c2->Print("DEMP_reco_18on275.pdf");
  c3b->Print("DEMP_reco_18on275.pdf");
  c3->Print("DEMP_reco_18on275.pdf");
  c3a->Print("DEMP_reco_18on275.pdf");
  c3c->Print("DEMP_reco_18on275.pdf");
  c4->Print("DEMP_reco_18on275.pdf");
  c4a->Print("DEMP_reco_18on275.pdf");
  c5->Print("DEMP_reco_18on275.pdf");
  c5_a->Print("DEMP_reco_18on275.pdf");
  c5a->Print("DEMP_reco_18on275.pdf");
  c5b->Print("DEMP_reco_18on275.pdf");
  c5c->Print("DEMP_reco_18on275.pdf");
  c5d->Print("DEMP_reco_18on275.pdf");
  c6b->Print("DEMP_reco_18on275.pdf");
  c6a->Print("DEMP_reco_18on275.pdf");
  c7->Print("DEMP_reco_18on275.pdf");
  c7a->Print("DEMP_reco_18on275.pdf");
  c7b->Print("DEMP_reco_18on275.pdf");
  c8->Print("DEMP_reco_18on275.pdf");
  c8a->Print("DEMP_reco_18on275.pdf");
  c9->Print("DEMP_reco_18on275.pdf");
  c9a->Print("DEMP_reco_18on275.pdf");
  c10->Print("DEMP_reco_18on275.pdf");
  c10a->Print("DEMP_reco_18on275.pdf");
  c11->Print("DEMP_reco_18on275.pdf");
  c11a->Print("DEMP_reco_18on275.pdf");
  c12->Print("DEMP_reco_18on275.pdf");
  c13->Print("DEMP_reco_18on275.pdf");
  c14a->Print("DEMP_reco_18on275.pdf");
  c14b->Print("DEMP_reco_18on275.pdf");
  c14c->Print("DEMP_reco_18on275.pdf");
  c14d->Print("DEMP_reco_18on275.pdf");
  c15a->Print("DEMP_reco_18on275.pdf");
  c15b->Print("DEMP_reco_18on275.pdf");
  c15c->Print("DEMP_reco_18on275.pdf");
  c15d->Print("DEMP_reco_18on275.pdf");
  c15e->Print("DEMP_reco_18on275.pdf");
  c15f->Print("DEMP_reco_18on275.pdf");
  c16f->Print("DEMP_reco_18on275.pdf");
  c16d->Print("DEMP_reco_18on275.pdf");
  c16e->Print("DEMP_reco_18on275.pdf");
  c16a->Print("DEMP_reco_18on275.pdf");
  c16b->Print("DEMP_reco_18on275.pdf");
  c16c->Print("DEMP_reco_18on275.pdf");
  c17->Print("DEMP_reco_18on275.pdf");
  c18b->Print("DEMP_reco_18on275.pdf");
  c18a->Print("DEMP_reco_18on275.pdf");
  c18->Print("DEMP_reco_18on275.pdf");
  c19a->Print("DEMP_reco_18on275.pdf");
  c19b->Print("DEMP_reco_18on275.pdf");
  c19->Print("DEMP_reco_18on275.pdf");
  c20a->Print("DEMP_reco_18on275.pdf");
  c20b->Print("DEMP_reco_18on275.pdf");
  c20->Print("DEMP_reco_18on275.pdf");
  c21a->Print("DEMP_reco_18on275.pdf");
  c21b->Print("DEMP_reco_18on275.pdf");
  c21->Print("DEMP_reco_18on275.pdf");
  c22a->Print("DEMP_reco_18on275.pdf");
  c22b->Print("DEMP_reco_18on275.pdf");
  c22->Print("DEMP_reco_18on275.pdf");
  c23a->Print("DEMP_reco_18on275.pdf");
  c23b->Print("DEMP_reco_18on275.pdf");
  c23c->Print("DEMP_reco_18on275.pdf");
  for(int D = 0; D <8; D++) { 
    c24[D]->Print("DEMP_reco_18on275.pdf"); }
  for(int D = 0; D <8; D++) {
    c25[D]->Print("DEMP_reco_18on275.pdf");};
  for(int D = 0; D <8; D++) {
    c26[D]->Print("DEMP_reco_18on275.pdf");};
  c27a->Print("DEMP_reco_18on275.pdf");
  c27b->Print("DEMP_reco_18on275.pdf");
  c28a->Print("DEMP_reco_18on275.pdf");
  c28b->Print("DEMP_reco_18on275.pdf");
  c28c->Print("DEMP_reco_18on275.pdf");
  c28d->Print("DEMP_reco_18on275.pdf");
  c28e->Print("DEMP_reco_18on275.pdf");
  c29->Print("DEMP_reco_18on275.pdf");
  c30a->Print("DEMP_reco_18on275.pdf");
  c1a->Print("DEMP_reco_18on275.pdf]");
  ...
  ofstream myfile;
  myfile.open("parameters_18on275.csv", std::ios::trunc);
